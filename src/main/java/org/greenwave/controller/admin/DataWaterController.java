...
@Controller
public class DataWaterController {
	...
	@RequestMapping("/dataExportWater")
	public String export(Map<String, Object> model, HttpSession session, ExportDataForm exportDataForm) {
		...
		List<String> ot_measures = measBusiness.listDataTypeByDomainAndCategory("OTHER_SUBSTANCES",dd[0]);
		List<String> po_measures = measBusiness.listDataTypeByDomainAndCategory("POLLUTING_SUBSTANCES",dd[0]);
		List<String> departments = locBusiness.findAllDepartement();
		...
		return "/admin/data/exportWater";
	}
	
	@RequestMapping("/dataExportWaterValidate")
	public String validateExport(Map<String, Object> model, HttpSession session, ExportDataForm exportDataForm) {
		...
		List<String> ot_measures = measBusiness.listDataTypeFromOther("OTHER_SUBSTANCES");
		List<String> po_measures = measBusiness.listDataTypeFromOther("POLLUTING_SUBSTANCES");
		List<String> departments = locBusiness.findAllDepartement();
		...
		if (exportDataForm.getFile_type().contains("Excel"))
		{
	        ...
	        for (int i = 0; i < datas_fetched.size(); i++){
		    	String [] entries = {datas_fetched.get(i).getId_data().toString(), 
		    					   ...
		    					   datas_fetched.get(i).getUser().getlName() + datas_fetched.get(i).getUser().getfName()};
		    	datas.add(entries);
	        }
	        int rowCount = 0;
	...
	@RequestMapping("/mapInteractiveWater")
	public String interactiveMap(Map<String, Object> model, HttpSession session, DateForm dateForm) {
		...
		model.put("username", username);
		DataDomain[] dd = DataDomain.values();
		List<Data> datas_tab = dataBusiness.findLastDataPerCoords(dd[0]);
		List<String> ot_measures = measBusiness.listDataTypeByDomainAndCategory("OTHER_SUBSTANCES", dd[0]);
		List<String> po_measures = measBusiness.listDataTypeByDomainAndCategory("POLLUTING_SUBSTANCES", dd[0]);
		ot_measures.addAll(po_measures);
		model.put("ot_measures", ot_measures);
		
		// convert to JSON for client-side processing
		String json_data = datasToJson(datas_tab);
		model.put("datas", json_data);

		
		return "/admin/map/interactiveWater";
	}
	
	@RequestMapping("/mapChangeWater")
	public String changeMap(Map<String, Object> model, HttpSession session, DateForm dateForm) {
		...
		try{
			...
			DataDomain[] dd = DataDomain.values();
			List<Data> datas_tab = dataBusiness.findBetweenDate(sDate, eDate,dd[0]);
			List<String> ot_measures = measBusiness.listDataTypeByDomainAndCategory("OTHER_SUBSTANCES", dd[0]);
			List<String> po_measures = measBusiness.listDataTypeByDomainAndCategory("POLLUTING_SUBSTANCES", dd[0]);
			ot_measures.addAll(po_measures);
			model.put("ot_measures", ot_measures);
			...
			if (dateForm.getMethod().equals("last"))
			{
				...
				// convert to JSON for client-side processing
				json_data = datasToJson(datas_tab);
				model.put("datas", json_data);
			}
	...
	@RequestMapping("/waterCharts")
	public String apiSend(Map<String, Object> model, ChartForm chartDateForm, HttpSession session) {
		...
		typesPolluting =  measBusiness.listDataTypeByDomainAndCategory("POLLUTING_SUBSTANCES", dd[0]);
		System.out.println(typesPolluting);
		List<String> typesClassics= new ArrayList<String>();
		typesClassics = measBusiness.listDataTypeByDomainAndCategory("OTHER_SUBSTANCES", dd[0]);

		List<String> locList= new ArrayList<String>();
		locList.addAll(locBusiness.findAllDepartement());
		
		
		model.put("deplist", locList);
		model.put("typeslistclassics",typesClassics);
		model.put("typeslistpolluting",typesPolluting);
		List<DataCategory> cat= dataCategoryBusiness.findByDomain(dd[0]);
		model.put("type",cat);
		
		return "/admin/charts/chartsWater";
	}		
	
	@RequestMapping("/changeWaterCharts")
	public String changeAirCharts(Map<String, Object> model,@Valid ChartForm chartDateForm, HttpSession session) {
		...
		model.put("typeslistclassics",typesClassics);
		model.put("typeslistpolluting",typesPolluting);
		model.put("waterCatId", dataCategoryBusiness.findByDomain(dd[0]));
		List<DataCategory> cat= dataCategoryBusiness.findByDomain(dd[0]);
		model.put("type",cat);
		...
		idsSelected=locBusiness.listIdLocationFromDepName(chartDateForm.getPlace());
